...
module NITTA.Test.Microarchitectures
    ...
    , runTargetSynthesis'
    , microarch, IOUnit(..)
    , pInt, pIntX32, pIntX48, pIntX128, pFX22_32, pFX42_64
    ) where
...
march = busNetwork 31 (Just True)
    [ ("fram1", PU def def FramPorts{ oe=SignalTag 0, wr=SignalTag 1, addr=map SignalTag [2, 3, 4, 5] } )
    , ("fram2", PU def def FramPorts{ oe=SignalTag 6, wr=SignalTag 7, addr=map SignalTag [8, 9, 10, 11] } )
    ...
    , ("mul", PU def (multiplier True) MultiplierPorts{ wr=SignalTag 22, wrSel=SignalTag 23, oe=SignalTag 24 } )
    , ("div", PU def (divider 8 True) DividerPorts{ wr=SignalTag 25, wrSel=SignalTag 26, oe=SignalTag 27, oeSel=SignalTag 28 } )
    ]
