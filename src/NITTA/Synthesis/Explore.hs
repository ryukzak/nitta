    ...
    let edges =
            concat
                ( map (decisionAndContext parent) (sAllocationOptions sState)
                    ...
                    ++ map (decisionAndContext parent) (sOptimizeLogicalUnitOptions sState)
...
nodeCtx parent nModel =
    let sBindOptions = bindOptions nModel
     ...
     in SynthesisState
            ...
            , sOptimizeLogicalUnitOptions = optimizeLogicalUnitOptions nModel
            , bindingAlternative =
                foldl
                    ( \st b -> case b of
