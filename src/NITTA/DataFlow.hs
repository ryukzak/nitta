...
data DataFlowGraph v
  = DFGNode (FB (Parcel v Int)) -- ^ вершина графа, соответствует фунциональному блоку.
...
-- * Ветвление алгоритма.
--
-- Под ветвлением алгоритма понимается возможность выбора одного из подграфов DFG `DFGSwitch(
-- dfgCases)` в зависимости от данных. При этом выбор подграфа может осуществляться 1) спекулятивно
-- (мультиплексор), если функциональные блоки не содержат побочных эффектов или 2) реальным условным
-- переходом практических как в неймановских процессорах.
