...
-- |Данный класс типов отвечает за привязку функциональных блоков к вычислительным. Он осуществляет
...
instance ( Var v, Time t, Typeable x
    ...
    -- 2. К состоянию модели вычислительного блока применяется решение относительно дальшейшего
    --    развития вычислительного процесса (преобразование варианта полученного из 'options'
    --    осуществляется за пределами модели) путём вызова фунции 'decision'.
