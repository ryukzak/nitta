...
module NITTA.Model.ProcessorUnits.NewAccum
  ( Accum
  , accum
  -- , Ports(..), IOPorts(..)
  ) where
...
instance UnambiguouslyDecode (Accum v x t) where
  decodeInstruction (Load A) = def{ initSignal=True, loadSignal=True, negSignal=Just False }
  decodeInstruction (Load B) = def{ loadSignal=True, negSignal=Just True }
