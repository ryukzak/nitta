...
-- | Решение об использовании вычислительных блоков в роли источника или пункта назначения для данных. В
...
class UnambiguouslyDecode pu where
  ...
  --
  -- Не может быть реализована для инструкций сетей, так как:
  -- * инструкция для сетей описывает несколько тактов, а значит должна возвращать последовательность
  -- значений микрокода;
