        ...
        {this.state.selectedNId !== null && (
          <div>
            <div className="tiny button-group">
              ...
              <a
                className="button primary"
                onClick={() => this.handleViewChange(this.state.selectedNId, this.state.synthesisStatus, "process")}
              >
                process
              </a>
              <a
                className="button primary"
                onClick={() => this.handleViewChange(this.state.selectedNId, this.state.synthesisStatus, "edges")}
              >
                edges
              </a>
              <a
                className="button primary"
                onClick={() =>
                  this.handleViewChange(this.state.selectedNId, this.state.synthesisStatus, "endpointOptions")
                }
              >
                endpointOptions
              </a>
              <a
                className="button primary"
                onClick={() => this.handleViewChange(this.state.selectedNId, this.state.synthesisStatus, "history")}
              >
                history
              </a>
              ...
              <a
                className="button primary"
                onClick={() => this.handleViewChange(this.state.selectedNId, this.state.synthesisStatus, "testbench")}
              >
                testbench
              </a>
            </div>
            <div className="tiny button-group">
              <a className="button primary" onClick={() => this.simpleSynthesis(this.state.selectedNId)}>
                simple synthesis
              </a>
              <a className="button primary" onClick={() => this.smartBindSynthesisIO(this.state.selectedNId)}>
                smart bind synthesis
              </a>
            </div>
            <div className="tiny button-group">
              <a className="button primary" onClick={() => this.obviousBindThread(this.state.selectedNId)}>
                oblious bind thread
              </a>
              <a className="button primary" onClick={() => this.allBestThread(this.state.selectedNId, 0)}>
                best thread
              </a>
              <a className="button primary" onClick={() => this.allBestThread(this.state.selectedNId, 1)}>
                all best thread 1
              </a>
              <a className="button primary" onClick={() => this.allBestThread(this.state.selectedNId, 2)}>
                all best thread 2
              </a>
            </div>
            ...
            {this.state.view === "process" && <ProcessView nId={this.state.selectedNId} />}
            {this.state.view === "edges" && (
              <EdgesView nid={this.state.selectedNId} onNidChange={(nid: any) => this.onNIdChange(nid)} />
            )}
    ...
    return (
      <div>
        ...
        <pre className="squeeze">
          {dump.tbCompilerDump.map((e: string, i: number) => (
            <div key={i}>
              {e}
              <br />
            </div>
        ...
        <pre className="squeeze">
          {dump.tbSimulationDump.map((e: string, i: number) => (
            <div key={i}>
              {e}
              <br />
            </div>
