...
export default class SynthesisGraphView extends React.Component<ISynthesisGraphViewProps, ISynthesisGraphViewState> {
    ...
    componentDidMount = () => {
        if (this.containerRef.current != null && this.buttonRef.current != null) {
            this.setState({ synthesisGraphHeight: this.containerRef.current.clientHeight - this.buttonRef.current.clientHeight });
        }
    }
            ...
            <div className="flex-grow-1" ref={this.containerRef}>
                <div className="d-flex justify-content-between my-1  mx-2" ref={this.buttonRef}>
                    <div className="d-flex flex-row my-auto">
                        <Button className="btn btn-link btn-sm bg-transparent mr-3" style={{ border: 0 }} onClick={() => this.resizeSynthesisGraphView(true)}>Expand</Button>
                        <Button className="btn btn-link btn-sm bg-transparent mr-3" style={{ border: 0 }} onClick={() => this.resizeSynthesisGraphView(false)}>Reduse</Button>
                        <Button className="btn btn-link btn-sm bg-transparent mr-3" style={{ border: 0 }} onClick={() => this.state.refreshGraph()}>Refresh</Button>
                    </div>
                    <div className="d-flex flex-row-reverse my-auto">
                        <h6>black - processed node; white - in progress node; green - succees synthesis</h6>
                    </div>
                </div>
                <div className="justify-content-center bg-light border" style={{ height: this.state.synthesisGraphHeight, minHeight: this.state.minSynthesisGraphHeight}} >
