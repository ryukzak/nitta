...
class SingleMetricCollector:
    ...
    def __str__(self):
        return f"{type(self).__name__}({self.name}: n={self.n}, mean={self.compute_mean()}, " \
...
class LeafMetricsCollector:
    ...
    def __init__(self):
        self.collectors: Dict[str, SingleMetricCollector] = {
            metric: SingleMetricCollector(metric)
            for metric in ("duration", "depth")
        }
    ...
    def collect_leaf_node(self, node: NittaNode):
        ...
        assert node.is_finish, "Shouldn't collect metrics from unsuccessful synthesis leafs"
