...
def test_df_to_model_columns_conversion_introduces_columns():
    ...
    assert df_to_model_columns(df_inp, model_columns=["a", "b", "c"]).equals(
        df_out_expected
    )
...
def test_df_to_model_columns_conversion_reorders_columns():
    ...
    assert df_to_model_columns(df_inp, model_columns=["a", "c", "b"]).equals(
        df_out_expected
    )
...
def test_df_to_model_columns_conversion_fills_missing_data():
    df_inp = pd.DataFrame(
        {"a": [1, 2, 3], "b": [4, pd.NA, pd.NA], "c": [pd.NA, pd.NA, 1]}
    )
